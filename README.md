...
Для дообучения использовалась предобученная модель sberbank-ai/rugpt3small_based_on_gpt2. 
Выбор модели обоснован тем, что она сочетает в себе предобучение на больших корпусах русского языка, возможность дообучения на своих данных, экономию вычислительных ресурсов за счет использования небольшой версии модели, а также высокую эффективность в задаче генерации текстовых отзывов на основе предоставленных параметров.
Дообучение проводилось на мощностях Google Colab Pro с использованием GPU Tesla A100, что позволило значительно ускорить процесс обучения и сократить время выполнения задач.
